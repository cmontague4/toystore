<?php   										// Opening PHP tag
	...
	function get_toy(PDO $pdo, string $id) {
		...
		// toy and manuf both have a name field, so we need to alias them as toyname and manufname
		$sql = "SELECT toy.*, manuf.*, toy.name AS toyname, manuf.name AS manufname
				FROM toy INNER JOIN manuf ON toy.manid = manuf.manid
				WHERE toy.toynum= :id";
